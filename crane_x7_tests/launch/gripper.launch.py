...
def generate_test_description():
    ...
    rosbag_cmd = ['ros2', 'bag', 'record', '/observation_camera/image',  '-o', rosbag_filepath]
    bag_recorder = ExecuteProcess(
        cmd=rosbag_cmd,
        output='screen', env=proc_env)
    sim = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(['crane_x7_gazebo',
         '/launch/crane_x7_with_table.launch.py'])
        )
    controller = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(['crane_x7_examples',
...
class TestAfterShutdown(unittest.TestCase):
    def test_exit(self, proc_info, rosbag_filepath):
        ...
        args = ['./ros2bag2video.py', '--topic', '/observation_camera/image', '--fps', '20','--rate', '1.0', '-o', output_video_name, rosbag_filepath]
        args = ['python', './ros2bag2video.py', '--topic', '/observation_camera/image', '--fps', '20','--rate', '1.0', '-o', output_video_name , rosbag_filepath]
        #'--fps', '2', 
        try:
        ...
        try:
            ...
            subprocess.run(args)
            subprocess.run(
                ['ffmpeg', '-i', output_video_name,
                 '-c:v', 'libvpx-vp9', '-crf', '30', '-b:v', '0', '-y', output_video_name.split(".")[0] + ".webm"])
        except Exception as e:
            print("error")
            print(e)
